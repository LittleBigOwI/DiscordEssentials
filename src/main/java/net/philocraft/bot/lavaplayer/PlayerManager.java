package net.philocraft.bot.lavaplayer;
...
public class PlayerManager {
    ...
    public void pause(SlashCommandInteractionEvent event) {
        ...
        if(!musicManager.isPaused()) {
            musicManager.pause();
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_PAUSE.get() + " Paused.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_ERROR.get() + " Already paused.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void resume(SlashCommandInteractionEvent event) {
        ...
        if(musicManager.isPaused()) {
            musicManager.resume();
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_RESUME.get() + " Resumed.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_ERROR.get() + " Already resumed.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void stop(SlashCommandInteractionEvent event) {
        ...
        if(musicManager.isPlaying()) {
            ...
            musicManager.stop();
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_STOP.get() + " Stopped.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_ERROR.get() + " Nothing is playing.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void shuffle(SlashCommandInteractionEvent event) {
        ...
        if(musicManager.scheduler.queue.isEmpty()) {
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_ERROR.get() + " The queue is empty.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
            musicManager.scheduler.shuffle();
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_SHUFFLE.get() + " Shuffled.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void loop(SlashCommandInteractionEvent event) {
        ...
        if(!musicManager.scheduler.isLooping()) {
            ...
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_LOOP.get() + " Enabled looping.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
            musicManager.scheduler.loop(false);
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_LOOP.get() + " Disabled looping.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void skip(SlashCommandInteractionEvent event) {
        ...
        if(musicManager.scheduler.queue.isEmpty()) {
            ...
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_SKIP.get() + " Skipped.").setColor(new Color(0x5865f2)).build()).queue();
        }
    }
    ...
    public void remove(SlashCommandInteractionEvent event, int index) {
        ...
        if(musicManager.scheduler.queue.size() > index || musicManager.scheduler.queue.isEmpty()) {
            event.replyEmbeds(new EmbedBuilder().setDescription(Emojis.MUSIC_ERROR.get() + " Index out of bounds.").setColor(new Color(0x5865f2)).build()).queue();
        } else {
